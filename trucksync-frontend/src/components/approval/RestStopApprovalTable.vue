...
const emit = defineEmits({
  approve: restStop =>
    restStop?.user_id !== undefined && restStop?.user_id !== null
});
...
const columns = computed(() => [
  ...
  {
    name: 'worksFrom',
    label: t('approval.restStops.worksFrom'),
    field: 'worksFrom',
    align: 'left',
    sortable: true
  },
  {
    name: 'worksTo',
    label: t('approval.restStops.worksTo'),
    field: 'worksTo',
    align: 'left',
    sortable: true
  },
...
const rows = computed(() =>
  props.restStops.map(restStop => ({
    ...
    profileId: restStop.id,
    userId: restStop.user_id,
    fullName: formatName(restStop.user),
    firstName: formatValue(restStop.user?.first_name),
    lastName: formatValue(restStop.user?.last_name),
    email: formatValue(restStop.user?.email),
    phoneNumber: formatValue(restStop.user?.phone_number),
    country: formatValue(restStop.user?.country),
...
const restStopCount = computed(() => props.restStops.length);
...
function isApproving(restStop) {
  return String(props.approvingUserId) === String(restStop.user_id);
}
    ...
    >
      <div class="col">
        <h2 class="text-h6 text-weight-bold q-my-none">
          {{ t('approval.restStops.title') }}
        </h2>
      </div>

      <div class="col-auto approval-muted text-caption text-weight-bold">
        {{ t('approval.restStops.count', { count: restStopCount }) }}
      </div>
    </q-card-section>
    ...
    >
      ...
      <template #no-data>
        ...
        >
          ...
          />
          <div class="column">
            <strong>{{ t('approval.restStops.emptyTitle') }}</strong>
            <span>{{ t('approval.restStops.emptyDescription') }}</span>
          </div>
        </div>
      </template>
    </q-table>
